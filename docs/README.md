# Docs

Minimal docs set for openboa's docs-driven development entrypoint.
Mintlify navigation is configured in `docs/docs.json`, and theme styling is defined in `docs/style.css`.

## Pages

- [Introduction](./introduction.md) - High-level project orientation and scope.
- [Quickstart](./quickstart.md) - Minimal local setup and docs validation flow.
- [Development](./development.md) - Core commands and recommended contribution workflow.
- [Single-Agent Runtime (Pi)](./runtime-single-agent-pi.md) - Minimal Pi-backed runtime path and storage boundaries.
- [Docs Portal](./index.md) - Entry point with key links and current docs status.
- [Business as Agent](./concepts/business-as-agent.md) - BOA definition and core building blocks.
- [Network hub](./network.md) - Governance-aware network documentation entrypoint.
- [Color Foundation](./foundation/colors.md) - Green/Gray/Sand palettes and semantic token mapping.
- [FAQ](./help/faq.md) - Common questions about scope, stage, and intended use.
- [Docs Troubleshooting](./help/troubleshooting-docs.md) - How to fix common docs link and structure issues.
- [한국어 문서 개요](./ko/README.md) - 한국어 문서 인덱스.
- [한국어 문서 홈](./ko/index.md) - 한국어 시작 문서 진입점.
- [한국어 소개](./ko/introduction.md) - 프로젝트 소개와 읽기 순서.
- [한국어 빠른 시작](./ko/quickstart.md) - 로컬 검증 최소 절차.
- [한국어 개발 가이드](./ko/development.md) - 핵심 명령과 작업 루프.
- [한국어 BOA 개념](./ko/concepts/business-as-agent.md) - BOA 모델과 구성 블록.
- [한국어 Core Doctrine](./ko/concepts/core-doctrine.md) - BOA-0 핵심 원칙.
- [한국어 System Contracts](./ko/concepts/system-contracts.md) - 철학을 스펙으로 내린 계약.
- [한국어 Network](./ko/network.md) - 네트워크 거버넌스 문서 허브.
- [한국어 Color Foundation](./ko/foundation/colors.md) - 색상 토큰 기준.
- [한국어 FAQ](./ko/help/faq.md) - 자주 묻는 질문.
- [한국어 Non-goals](./ko/help/non-goals.md) - 명시적 제외 항목.
- [한국어 Docs Troubleshooting](./ko/help/troubleshooting-docs.md) - 문서 검증/문제 해결.
