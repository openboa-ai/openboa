...
<Steps>
  ...
  <Step title="Mintlify 라우팅/구조 검증">
    `cd docs && pnpm dlx mintlify broken-links` 및 `cd docs && pnpm dlx mintlify validate` 실행
  </Step>
  <Step title="핵심 경로 스팟체크">
    [문서 홈](/ko/index), [FAQ](/ko/help/faq), [Business as Agent](/ko/concepts/business-as-agent) 접근 확인
  </Step>
</Steps>
