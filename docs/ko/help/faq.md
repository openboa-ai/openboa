...
<AccordionGroup>
  <Accordion title="openboa는 무엇인가요?">
    openboa는 Business as Agent(BOA) 시스템입니다. Business를 지속되는 운영 주체로 두고, Agent를 진화 가능한 실행 인력으로 다룹니다.
  </Accordion>
  ...
  <Accordion title="어디부터 읽으면 좋나요?">
    Introduction, Business as Agent, Core Doctrine, Architecture 순으로 읽고, 그 다음 Agent, Chat, Work, Observe surface 문서를 읽는 것을 권장합니다.
  </Accordion>
</AccordionGroup>
