...
export const DEFAULT_ALLOWED_AGENT_IMPORT_PREFIXES = ["src/agents/", "src/foundation/"]
...
export async function findAgentImportBoundaryViolations(options = {}) {
  const repoRoot = options.repoRoot ?? path.resolve(__dirname, "..")
  const agentsRoot = path.resolve(repoRoot, "src/agents")
  const allowedPrefixes = options.allowedPrefixes ?? DEFAULT_ALLOWED_AGENT_IMPORT_PREFIXES

  const files = await listSourceFiles(agentsRoot)
  const violations = []
...
export async function assertAgentImportBoundary(options = {}) {
  const violations = await findAgentImportBoundaryViolations(options)
  ...
  const rendered = violations
    ...
    .join("\n")
  throw new Error(`agent import boundary violations detected:\n${rendered}`)
}

if (process.argv[1] === __filename) {
  try {
    await assertAgentImportBoundary()
    console.log("Agent import boundary passed.")
  } catch (error) {
