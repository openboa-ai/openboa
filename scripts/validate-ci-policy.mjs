...
import { assertAgentImportBoundary } from "./check-agent-boundary.mjs"
import { assertChatImportBoundary } from "./check-chat-boundary.mjs"
import { assertChatLegacyNaming } from "./check-chat-legacy-naming.mjs"
import { assertShellChatImportBoundary } from "./check-shell-chat-boundary.mjs"
import { assertWebChatImportBoundary } from "./check-web-chat-boundary.mjs"
...
if (process.argv[1] === __filename) {
  ...
  try {
    ...
    await assertAgentImportBoundary({ repoRoot })
    await assertChatImportBoundary({ repoRoot })
    await assertChatLegacyNaming({ repoRoot })
    await assertShellChatImportBoundary({ repoRoot })
    await assertWebChatImportBoundary({ repoRoot })
