...
export async function syncCodexOauthFromCodexCli(
  ...
  const authDir = join(companyDir, ".openboa", "auth")
...
export async function runCodexOauthLoginAndSync(companyDir: string): Promise<CodexOauthSyncResult> {
  await runCodexLoginCommand()
  return syncCodexOauthFromCodexCli(companyDir, resolveCodexCliAuthPath())
}
