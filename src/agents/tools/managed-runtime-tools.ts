...
async function _listScopedSessions(input: {
  session: Session
  sessionStore: SessionStore
  ...
  const sessions = await input.sessionStore.listAgentSessions(input.session.agentId)
  const requestedIds =
    input.requestedSessionIds && input.requestedSessionIds.length > 0
      ? new Set(input.requestedSessionIds)
      : null

  return sessions
    .filter((session) => (input.includeCurrent === false ? session.id !== input.session.id : true))
    .filter((session) => (requestedIds ? requestedIds.has(session.id) : true))
}
    ...
    if (tool.permissionPolicy === "always_ask") {
      const sameToolPendingRequest =
        input.pendingToolConfirmationRequest?.toolName === tool.name
          ...
          : null
      const _matchedPendingRequest =
        sameToolPendingRequest && sameStructuredValue(sameToolPendingRequest.input, args ?? {})
