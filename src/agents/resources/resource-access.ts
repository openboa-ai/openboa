...
export async function stageSubstrateArtifactToSessionWorkspace(input: {
  ...
  const sourceContent = await readRegularFile(source.actualPath, input.sourcePath)
  ...
  await withResourceWriteLease(
    ...
    async () => {
      ...
      const existingContent = await readFileIfExists(target.actualPath)
      if (existingContent && !input.overwrite) {
        ...
        await writeFile(target.actualPath, sourceContent)
      }
      if (!reusedExisting) {
  ...
  try {
    ...
    if (error instanceof Error && "code" in error) {
      if (error.code === "ENOENT") {
        throw new Error(`Source path ${label} does not exist`)
      }
      if (error.code === "EISDIR") {
        throw new Error(`Source path ${label} is not a file`)
      }
    }
    throw error
  }
