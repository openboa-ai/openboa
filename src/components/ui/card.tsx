...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-3 rounded-[var(--radius-card)] border border-border bg-card py-3 text-card-foreground shadow-[var(--shadow-card)]",
        className,
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("leading-[1.25] font-semibold tracking-[-0.04em]", className)}
      {...props}
    />
  )
}
